...
TEST_F(TorrentsTest, invalidArgsAreLogged)
{
    ...
    tr_torrentSetSpeedLimit_KBps(nullptr, tr_direction::Up, 0);
    ++expected_log_size;

    EXPECT_EQ(0, tr_torrentGetSpeedLimit_KBps(nullptr, tr_direction::Up));
    ++expected_log_size;

    tr_torrentUseSpeedLimit(nullptr, tr_direction::Up, true);
    ++expected_log_size;

    EXPECT_FALSE(tr_torrentUsesSpeedLimit(nullptr, tr_direction::Up));
    ++expected_log_size;
    ...
    EXPECT_EQ(""sv, tr_torrentGetDownloadDir(nullptr));
    ++expected_log_size;

    EXPECT_EQ(""sv, tr_torrentGetCurrentDir(nullptr));
    ++expected_log_size;
    ...
    EXPECT_FALSE(tr_torrentSetTrackerList(nullptr, ""sv));
    ++expected_log_size;
