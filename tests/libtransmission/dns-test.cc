...
TEST_F(EvDnsTest, canLookup)
{
    ...
    dns.lookup("example.com", [&done](struct sockaddr const* ai, socklen_t ailen, time_t expires_at) {
...
TEST_F(EvDnsTest, canRequestWhilePending)
{
    ...
    dns.lookup("example.com", [&n_done](struct sockaddr const* ai, socklen_t ailen, time_t expires_at) {
        EXPECT_NE(nullptr, ai);
        EXPECT_GT(ailen, 0);
        EXPECT_GT(expires_at, tr_time());
        ++n_done;
...
TEST_F(EvDnsTest, canCancel)
{
    ...
    auto tag = dns.lookup(Name, [&n_done](struct sockaddr const* ai, socklen_t ailen, time_t expires_at) {
        ++n_done;
    ...
    dns.lookup(Name, [&n_done](struct sockaddr const* ai, socklen_t ailen, time_t expires_at) {
        ++n_done;
...
TEST_F(EvDnsTest, doesCacheEntries)
{
    ...
    dns.lookup(Name, [&ai_addr](struct sockaddr const* ai, socklen_t ailen, time_t expires_at) {
        EXPECT_NE(nullptr, ai);
        EXPECT_GT(ailen, 0);
        EXPECT_GT(expires_at, tr_time());
        ai_addr = ai;
    ...
    dns.lookup(Name, [&ai_addr, &second_callback_called](struct sockaddr const* ai, socklen_t ailen, time_t expires_at) {
        EXPECT_NE(nullptr, ai);
        EXPECT_GT(ailen, 0);
