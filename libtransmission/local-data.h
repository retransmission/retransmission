...
public:
     ...
     *
     * `Shuffled` is for testing. It parks callbacks until `pump()` is
     * called, then delivers them in a random order. The rules above
     * allow this, so a caller that breaks under it would also break
     * under a threaded backend.
     */
    ...
    // `wake` is called when the first completion is parked.
    // The owner should answer it by calling pump() from the session thread.
    void set_completions(Completions completions, std::function<void()> wake = {});
...
private:
    ...
    void finish(Fn&& fn)
    {
        if (completions_ == Completions::Inline) {
            fn();
            return;
        }
