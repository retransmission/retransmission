...
public:
    ...
    // session thread, with the outcome of the update that installs a list: if a
    // newer update() takes over first, `on_done` waits for that one instead. It
    // is not invoked at all if cancel() is called first.
