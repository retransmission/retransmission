...
// The names by which one client reaches another.
...
//
// These three are ours alone, not shared with Transmission: two products that answered on
// one bus name would fight over it, and a desktop matches a window to its launcher by the
// application id built from them. A caller finds the names to use in the running client's
// `dbus-peer` record rather than assuming its own, so handoff still crosses products.
// A client that publishes no record is reachable only by one built with these same names.
#define TR_INTEROP_DBUS_SERVICE_NAME "org.retransmission.Retransmission"
#define TR_INTEROP_DBUS_INTERFACE_NAME "org.retransmission.Retransmission"
#define TR_INTEROP_DBUS_OBJECT_PATH "/org/retransmission/Retransmission"
