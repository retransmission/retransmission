 ...
 * **disk_io_workers:** Number (default = 0) Number of background threads for reading torrent data from disk. 0 reads on Transmission's main network thread as older releases did. 2 is a good starting point for trying the threaded reader: seeding from a spinning disk gets much faster, and a slow disk no longer stalls the network or the UI. Changes take effect after a restart.
