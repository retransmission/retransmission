...
{
    for (SCNetworkInterfaceRef interface = SCNetworkServiceGetInterface(service); interface != nullptr; interface = SCNetworkInterfaceGetInterface(interface)) {
...
BOOL TRVPNServiceInfoMatchesIdentity(NSDictionary<NSString*, NSString*>* serviceInfo, NSString* expectedServiceName, NSString* expectedProviderIdentifier)
{
...
NSArray<NSDictionary<NSString*, id>*>* TRActiveNetworkInterfaceInfo()
{
    ...
    for (struct ifaddrs const* ifa = ifaddrs; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET || family == AF_INET6) {
            ...
            if (getnameinfo(ifa->ifa_addr, family == AF_INET ? sizeof(sockaddr_in) : sizeof(sockaddr_in6), host, sizeof(host), nullptr, 0, NI_NUMERICHOST) ==
                0) {
